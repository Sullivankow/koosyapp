...
export function BienCard({ bien, onOpen }: BienCardProps) {
  const ownerFullName = bien.proprietaire
    ...
    : '—'
  const ownerInitials = bien.proprietaire
    ? `${bien.proprietaire.prenom} ${bien.proprietaire.nom}`
        .split(' ')
        .map((n) => n[0])
        .join('')
    : ''
  const ownerEmail = bien.proprietaire?.email ?? ''
      ...
      <div className="flex-1 p-4 space-y-2 text-sm">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            >
              ...
              {bien.statut === 'occupé' && 'Occupé'}
              {bien.statut === 'travaux' && 'En travaux'}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center gap-2">
            ...
            <div className="space-y-0.5">
              <p className="text-xs font-medium text-[#1F2933]">{ownerFullName}</p>
              <p className="text-[11px] text-[#6E7B8B]">{ownerEmail}</p>
            </div>
          </div>
